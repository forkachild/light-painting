 ...
 * @date 2022-11-28
 *
 * Highly optimized SPI DMA based data transmission layer for WS2812 RGB
 * addressable LED strips.
 *
 * WS2812 RGB addressable LED chain driver. The SPI peripheral is used for the
 * transmission with the optional benefit of DMA. At 3.333333MHz, each "bit"
 * takes 0.33us(330ns), and complying with WS2812 documentation, the following
 * is the encoding
 *
...
/**
 * @brief The number of transmission-bytes required to invoke a "reset" in
 * WS2812.
 *
...
void ws2812_spi_driver_init(WS2812SpiDriver *driver, size_t ledCount) {
    if (ledCount <= 0) {
    ...
    size_t dataXferSize =
        ledCount * BYTES_PER_RGB_PIXEL * BYTES_ENCODING_MULTIPLIER;
    size_t totalXferSize = dataXferSize + BYTES_TRANSMITTED_FOR_RESET;
    driver->pDataBuffer = (uint16_t *)malloc(totalXferSize);
    driver->dmaChannel = dma_claim_unused_channel(true);
    memset((uint8_t *)(driver->pDataBuffer), 0, totalXferSize);

    dma_channel_config config =
        dma_channel_get_default_config(driver->dmaChannel);
    channel_config_set_transfer_data_size(&config, DMA_SIZE_8);
    channel_config_set_dreq(&config, spi_get_dreq(spi_default, true));
    channel_config_set_read_increment(&config, true);
    channel_config_set_write_increment(&config, false);

    dma_channel_configure(driver->dmaChannel, &config,
                          &spi_get_hw(spi_default)->dr, NULL, totalXferSize,
                          false);
}

void ws2812_spi_driver_submit_argb_buffer_blocking(
    WS2812SpiDriver *driver, const uint32_t *pArgbBuffer) {
    for (int i = 0, k = 0; i < driver->ledCount; i++) {
        ...
        driver->pDataBuffer[k++] = DICT[(green >> 4) & 0xF];
        driver->pDataBuffer[k++] = DICT[green & 0xF];

        driver->pDataBuffer[k++] = DICT[(red >> 4) & 0xF];
        driver->pDataBuffer[k++] = DICT[red & 0xF];

        driver->pDataBuffer[k++] = DICT[(blue >> 4) & 0xF];
        driver->pDataBuffer[k++] = DICT[blue & 0xF];
    }
    ...
    dma_channel_set_read_addr(driver->dmaChannel, driver->pDataBuffer,
                              true);
    dma_channel_wait_for_finish_blocking(driver->dmaChannel);
}

void ws2812_driver_deinit(WS2812SpiDriver *driver) {
